...
output_path = '/Users/ashwinchaubey/Documents/MATLAB/eeglab2024-2.0/Participants/Averaged3_files/';
csv_path = '/Users/ashwinchaubey/Documents/MATLAB/eeglab2024-2.0/Participants/Env1CSV_files/';
...
for i = 1:length(xdf_files)
    try
        ...
        % Read the CSV file and count the number of 'leave' decisions
        csv_file = fullfile(csv_path, [xdf_files(i).name(1:end-4), '.csv']);
        disp(['Reading CSV file: ' csv_file]);
        csv_data = readtable(csv_file);
        num_leave_trials = sum(strcmp(csv_data.Decision, 'leave'));
        disp(['Number of leave trials: ', num2str(num_leave_trials)]);
        ...
        EEG = pop_select(EEG, 'trial', [1:num_leave_trials]);
